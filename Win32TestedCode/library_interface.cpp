...
/**
 * a disgusting workaround for Windows' DLL limitation
 * on Windows you can't leave unresolved symbols in the binary (and LoadLibrary doesn't
 * do bidirectional linking), so we have to explicitly set the emulator instance
 * and only then can we call any methods on it
*/
extern "C" __declspec(dllexport) void set_arduino_emulator_instance(ArduinoEmulator * p_emulator) {
	emulator = p_emulator;
}
